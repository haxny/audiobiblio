...
5. Auto-download programs: first three downloads per program require user approval (`APPROVAL` status); subsequent downloads are `PENDING` immediately `[partial: threshold-based approval exists; per-target auto-vs-review switch not yet in CrawlTarget model]`
