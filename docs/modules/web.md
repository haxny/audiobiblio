...
| `GET /search` | Global search (`?q=`) across Works (title, author), Episodes (title, summary) and Programs (name) — dense three-section results page (Díla / Epizody / Pořady) with counts, each section capped at 50 rows with uncapped totals. Matching is case- and diacritics-insensitive (`unidecode`-normalized on both sides, so `hasek` finds `Hašek` and vice versa) via the pure helper `_query_search(db, q, limit=50)` (unit-testable like `_query_gaps`). Works link to their first episode's detail, episodes to `/episodes/{id}`, programs to `/episodes?q={name}`. Fed by a compact `.nav-search` form on the right side of the header nav in `base.html`. Empty/missing `q` renders an empty state. |
